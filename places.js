/**
 * Created by Peter on 2018/1/11 0011.
 */
Vue.component("place", {
    template:
        `<div>
   <select :class='place_class' v-model='p_' name="p" required="required" >
       <option disabled value="">请选择</option>
       <option v-for='(v, i) in this.cities' :key='"s1"+i' :value='i'>{{i}}</option>
   </select>
            
   <select :class='place_class'  v-model='c_' name="c" required="required">
       <option disabled value="" >请选择</option>
       <option v-for='(v , i) in c' :key='"s2"+i' :value='i'>{{i}}</option>
   </select>
            
    <select :class='place_class' v-model='a_' name="a" required="required">
       <option disabled value="">请选择</option>
       <option v-for='(v , i) in a' :key='"s3"+i' :value='v'>{{v}}</option>
    </select>
    <slot>
    </slot>
    ...
    created: function () {
        ...
        // console.log(this.p__);
    },
    watch: {
        //将父组件的值传个组件变量
        p_: function (new_, old_) {

            if(old_) this.c_="";
            this.c = this.cities[new_];

            console.log(123);


        },
        c_: function (new_, old_) {
         ...
         c__: function (new_, old_) {

            this.c_=new_;


        },
         a__: function (new_, old_) {
